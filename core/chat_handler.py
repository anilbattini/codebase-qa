...
class ChatHandler:
    ...
    def __init__(self, llm, project_config, project_dir=".", rewrite_llm=None):
        self.llm = llm
    ...
    def process_query(self, query, qa_chain, log_placeholder, debug_mode=False):
        ...
        try:
            ...
            try:
                enhanced_context = self.context_builder.build_enhanced_context(retrieved_docs, query, intent)
            ...
            # Phase 6: Generate final answer DIRECTLY with the main LLM using strict context prompt
            st.session_state.thinking_logs.append("🤖 Generating answer with strict context prompt...")
            update_logs(log_placeholder)
            log_to_sublog(self.project_dir, "chat_handler.log", "Generating answer with strict context prompt...")
