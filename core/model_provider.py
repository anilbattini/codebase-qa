...
class OllamaProvider(ModelProvider):
    ...
    def __init__(self, endpoint, embedding_model, llm_model):
...
class HuggingFaceProvider(ModelProvider):
    ...
    def __init__(self, embedding_model, llm_model):
        ...
        # Always use system cache - simpler and more reliable
        import os
        self.cache_dir = os.path.expanduser("~/.cache/huggingface")
        
        # Set environment variables for Hugging Face to use system cache
        os.environ['HF_HOME'] = self.cache_dir
        os.environ['SENTENCE_TRANSFORMERS_HOME'] = os.path.join(self.cache_dir, 'sentence_transformers')
        
        log_to_sublog(".", "model_provider.log", f"📁 Using system cache directory: {self.cache_dir}")
...
class LocalHuggingFaceEmbeddings:
    ...
    def _ensure_model_downloaded(self, model_name: str, model_type: str = "model"):
        """Ensure a model is downloaded locally to custom cache directory (cache-first, network only if missing)."""
        if model_name in self._downloaded_models:
            log_to_sublog(".", "model_provider.log", f"✅ Model already loaded: {model_name}")
            return
        ...
        try:
            ...
            if model_type == "embedding":
                ...
                if os.path.exists(model_cache_path):
                    log_to_sublog(".", "model_provider.log", f"✅ Model already cached: {model_name}")
                    try:
                        # Test load cached model
                        _ = SentenceTransformer(
                            model_name,
                            cache_folder=self.cache_dir,
                            device="cpu",
                        ...
                        log_to_sublog(".", "model_provider.log", f"✅ Successfully verified cached model: {model_name}")
                        self._downloaded_models.add(model_name)
                        return
                    except Exception as load_error:
                        log_to_sublog(".", "model_provider.log", f"⚠️ Cached model verification failed, re-downloading: {load_error}")
                
                log_to_sublog(".", "model_provider.log", f"📥 Downloading embedding model: {model_name}")
                
                # Download with CPU-only configuration to avoid meta tensor issues
                try:
                    ...
                    log_to_sublog(".", "model_provider.log", f"✅ Successfully downloaded embedding model: {model_name}")
                    self._downloaded_models.add(model_name)
                except Exception as e:
                    log_to_sublog(".", "model_provider.log", f"❌ Failed to download embedding model {model_name}: {e}")
                    raise Exception(f"Failed to download embedding model {model_name}: {e}")

            else:
                # LLM path (open models only, no token)
                from transformers import AutoTokenizer, AutoModelForCausalLM
                import torch

                # 1) Tokenizer cache-only probe
                if os.path.exists(model_cache_path):
                    try:
                        _ = AutoTokenizer.from_pretrained(model_name, cache_dir=self.cache_dir, local_files_only=True, trust_remote_code=True)
                        log_to_sublog(".", "model_provider.log", f"✅ Cache-only tokenizer OK for {model_name}")
                    except Exception as e:
                        log_to_sublog(".", "model_provider.log", f"⚠️ Cache-only tokenizer load failed: {e} — will online-load if needed")

                # 2) Load tokenizer and model (online if not cached)
                _ = AutoTokenizer.from_pretrained(model_name, cache_dir=self.cache_dir, trust_remote_code=True)
                ...
                self._downloaded_models.add(model_name)
                log_to_sublog(".", "model_provider.log", f"✅ LLM ready: {model_name}")

        except Exception as e:
            log_to_sublog(".", "model_provider.log", f"❌ Failed to ensure {model_type} {model_name}: {e}")
            raise Exception(f"Failed to download {model_type} {model_name}: {e}")
...
def set_provider(provider_type: str, **kwargs):
    ...
    try:
        ...
        try:
            _current_provider = ModelProviderFactory.create_provider("ollama", 
                                                                     endpoint="http://localhost:11434", 
                                                                     embedding_model="nomic-embed-text:latest", llm_model="llama3.1:latest")
            _current_provider_type = "ollama"
