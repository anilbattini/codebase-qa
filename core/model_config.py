...
class ModelConfig:
    ...
    def __init__(self):
        ...
        self._ollama_llm_model = "llama3.1:latest"
        self._ollama_embedding_model = "nomic-embed-text:latest"
    ...
    def _initialize_provider(self):
        ...
        try:
            ...
            if self._provider_type == "ollama":
                ...
                set_provider("ollama", endpoint=self._ollama_endpoint, embedding_model=self._ollama_embedding_model, llm_model=self._ollama_llm_model)
    ...
    def set_provider_type(self, provider_type: str):
        """Set the provider type and reinitialize."""
        try:
            log_to_sublog(".", "model_provider.log", f"🔄 Setting provider type to: {provider_type}")
            self._provider_type = provider_type
            self._initialized = False  # Reset initialization flag
            self._initialize_provider()
            log_to_sublog(".", "model_provider.log", f"✅ Provider type set to: {provider_type}")
        except Exception as e:
            log_to_sublog(".", "model_provider.log", f"❌ Failed to set provider type to {provider_type}: {e}")
            raise e
    ...
    def get_provider(self, provider_type: str = None):
        ...
        if provider_type == "ollama":
            ...
            return OllamaProvider(endpoint=self._ollama_endpoint, embedding_model=self._ollama_embedding_model, llm_model=self._ollama_llm_model)
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            "provider_type": self._provider_type,
            "ollama_model": self._ollama_llm_model,
    ...
    def load_config(self, config: Dict[str, Any]):
        ...
        if "ollama_model" in config:
            self._ollama_llm_model = config["ollama_model"]
