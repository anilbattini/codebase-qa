...
class UIComponents:
    ...
    def render_sidebar_config(self):
        ...
        # Return provider-aware values instead of hardcoded Ollama values
        current_provider_type = model_config.get_current_provider_type()
        if current_provider_type == "ollama":
            model_value = model_config.get_ollama_model()
            endpoint_value = model_config.get_ollama_endpoint()
        else:  # huggingface
            # For Hugging Face, we don't need endpoint, but we need to pass something
            # The build_rag function will ignore these when using Hugging Face
            model_value = model_config.get_huggingface_llm_model()
            endpoint_value = "huggingface://local"  # Placeholder for Hugging Face
        
        return project_dir, model_value, endpoint_value, force_rebuild, debug_mode
